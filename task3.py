import os
from pprint import pprint


def rewrite_file(path1=None, path2=None, path3=None): # Пишем функцию по записи в файл
  if path1 or path2 or path3 is None:
    path1 = '1.txt'
    path2 = '2.txt'
    path3 = '3.txt'
    os.chdir(r'C:\Users\danil\PycharmProjects\Netology\files') # Отмечаем, что находимся в этой папке
    outout_file = "rewrite_file.txt" # Это выходной файл, в котором будут содержаться результаты объединения
    file1_path = os.path.join(os.getcwd(), path1)  # Путь, используемый в данный момент
    file2_path = os.path.join(os.getcwd(), path2)
    file3_path = os.path.join(os.getcwd(), path3)
    with open(file1_path, 'r', encoding='utf-8') as f1: # Открываем файл file_path1 для чтения
      file1 = f1.readlines() # Читаем файл f1 по строкам
    with open(file2_path, 'r', encoding='utf-8') as f2:
      file2 = f2.readlines()
    with open(file3_path, 'r', encoding='utf-8') as f3:
      file3 = f3.readlines()
    with open(outout_file, 'w', encoding='utf-8') as f_total:

      if len(file1) < len(file2) and len(file1) < len(file3): # Занимаемся сортировкой
        f_total.write(path1 + '\n')
        f_total.write(str(len(file1)) + '\n')
        f_total.writelines(file1)
        f_total.write('\n')
        f_total.write('\n')
      elif len(file2) < len(file1) and len(file2) < len(file3):
        f_total.write(path2 + '\n')
        f_total.write(str(len(file2)) + '\n')
        f_total.writelines(file2)
        f_total.write('\n')
        f_total.write('\n')
      elif len(file3) < len(file1) and len(file3) < len(file2):
        f_total.write(path3 + '\n')
        f_total.write(str(len(file3)) + '\n')
        f_total.writelines(file3)
        f_total.write('\n')
        f_total.write('\n')
      if len(file2) > len(file1) > len(file3) or len(file2) < len(file1) < len(
              file3):
        f_total.write(path1 + '\n')
        f_total.write(str(len(file1)) + '\n')
        f_total.writelines(file1)
        f_total.write('\n')
        f_total.write('\n')
      elif len(file1) > len(file2) > len(file3) or len(file2) > len(file1) and len(file2) < len(
              file3):
        f_total.write(path2 + '\n')
        f_total.write(str(len(file2)) + '\n')
        f_total.writelines(file2)
        f_total.write('\n')
        f_total.write('\n')
      elif len(file1) > len(file3) > len(file2) or len(file3) > len(file1) and len(file3) < len(
              file2):
        f_total.write(path3 + '\n')
        f_total.write(str(len(file3)) + '\n')
        f_total.writelines(file3)
        f_total.write('\n')
        f_total.write('\n')
      if len(file1) > len(file2) and len(file1) > len(file3):
        f_total.write(path1 + '\n')
        f_total.write(str(len(file1)) + '\n')
        f_total.writelines(file1)
        f_total.write('\n')
        f_total.write('\n')
      elif len(file2) > len(file1) and len(file2) > len(file3):
        f_total.write(path2 + '\n')
        f_total.write(str(len(file2)) + '\n')
        f_total.writelines(file2)
        f_total.write('\n')
        f_total.write('\n')
      elif len(file3) > len(file1) and len(file3) > len(file2):
        f_total.write(path3 + '\n')
        f_total.write(str(len(file3)) + '\n')
        f_total.writelines(file3)
        f_total.write('\n')
        f_total.write('\n')
  else:
    print()
  return


rewrite_file()